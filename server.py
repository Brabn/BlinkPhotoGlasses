...
def clientthread(conn,devicenumber):
	...
	if (int(bufsize)>=2359316):		#max size for 1027x768 image = 2359317-1
			...
			full_string="".join(message)
                        #game.init()
			#print ("Recieved data "+str(sys.getsizeof(full_string)))
			...
			try:
				...
				#print ("image saved"+str(sys.getsizeof(rimage)))
				#print ("time "+str(currtime[currdevice]))
				#print ("from last "+str(comptime[currdevice]))
			except:
			...
			for d in range(1, devicecount+1):
				...
				if (abs(currtime[d-1]-currtime[d])<=interval):
					#print ("Two blink (devices "+str(d-1)+","+str(d)+") with interval "+str(currtime[d-1]-currtime[d]))
					showimage=True
					showimageN=d
			...
			if (devicecount>1):
				if (abs(currtime[0]-currtime[devicecount])<=interval):
					#print ("Two blink (devices "+str(0)+","+str(devicecount)+") with interval "+str(currtime[0]-currtime[devicecount]))
					showimage=True
					showimageN=d
			...
			#		
			#	if (abs(currtime[d-1]-currtime[d])<=interval):
			#		print ("Two blink with interval "+str(currtime[currdevice-1]-currtime[currdevice]))
			#		os.system('sudo fbi -T 1 -a -noverbose '+recievedimage+str(currdevice)+'.jpg') 	
			#if (abs(currtime[0]-currtime[devicecount-1])<=interval):
			string=""
			message=[]
			full_string=""
			bufsize=0;
...
#start_new_thread(showimg ,(backimage,))
#os.system('sudo fbi -T 1 -a -noverbose '+backimage)
#os.system('sudo fbi -T 1 -a -noverbose '+eyeimage)
#myProc = subprocess.Popen(["fbi", "--autoup", "--once", "-T", "1", "-noverbose", eyeimage])
os.system('sudo fbi -T 1 -a -noverbose '+eyeimage)
showimage=True
pygameinit=False
while 1:
    ...
    if showimage:
	#print "show image "+str(showimageN)
	#os.system('sudo fbi -T 1 -a -noverbose '+backimage)
	#os.system('sudo fbi -T 1 -a -noverbose '+eyeimage)
	#time.sleep(10)
	showimage=False
    ...
    #else:
    	#print "Duplicate IP "+str(devicenumber)
game.quit()
s.close()